...
class ChannelsConfig(Base):
    ...
    send_progress: bool = True  # stream agent progress to channel
...
class AgentDefaults(Base):
    ...
    provider: str = "auto"  # Provider name (e.g. "anthropic") or "auto"
    max_tokens: int = 8192
...
class WebSearchUserLocation(Base):
...
class Config(BaseSettings):
    ...
    def _match_provider(self, model: str | None = None) -> tuple["ProviderConfig | None", str | None]:
        ...
        forced = (self.agents.defaults.provider or "auto").replace("-", "_")
        if forced != "auto":
        ...
        # Match by keyword (order follows provider registry).
        for spec in PROVIDERS:
            provider = getattr(self.providers, spec.name, None)
        ...
        # Fallback: non-OAuth providers with keys, in registry order.
        for spec in PROVIDERS:
            if spec.is_oauth:
                continue
            provider = getattr(self.providers, spec.name, None)
    ...
    def get_api_base(self, model: str | None = None) -> str | None:
        """Get API base URL for the given model, with gateway defaults."""
    ...
    # Backward-compatible names kept for local CLI call-sites.
